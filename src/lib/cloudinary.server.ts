...
import { isCloudinaryConfigured, cloudName, type CloudinaryUploadResult } from './cloudinary';
...
async function getCloudinaryClient() {
  ...
  if (!isCloudinaryConfigured) {
    throw new Error('Cloudinary não está configurado. Defina as variáveis de ambiente necessárias.');
  }
...
export const uploadImage = async (
  ...
  const result = (await cloudinary.uploader.upload(imageData, uploadParams)) as CloudinaryUploadResult;
  return result;
};
