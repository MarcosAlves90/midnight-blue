...
import { validateDiceParams, rollSingleDie, generateNotation, calculateSelected } from './utils';
...
export function rollDice(config: Partial<RollConfig> = {}): DiceResult {
    ...
    // Execute dice rolls
    const rolls = Array.from({ length: totalDiceCount }, () => rollSingleDie(fullConfig.faces));
 ...
 */
export async function rollDiceAsync(config: Partial<RollConfig> = {}, delayMs = 0): Promise<DiceResult> {
    if (delayMs > 0) {
