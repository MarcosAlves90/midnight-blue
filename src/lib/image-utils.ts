...
export async function captureElementAsPng(element: HTMLElement, options?: Record<string, unknown>) {
  return new Promise<string>((resolve, reject) => {
    ...
    runInIdle(async () => {
      ...
      try {
        const dataUrl = await toPng(element, options as Record<string, unknown>);
        try {
          ...
          if (m && m.duration > 200) console.warn(`[perf] ${perfKey} took ${Math.round(m.duration)}ms`);
