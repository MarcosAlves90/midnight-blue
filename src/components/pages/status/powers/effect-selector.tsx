...
export const EffectSelector = memo(({ onSelect, onClose, excludeIds = [] }: EffectSelectorProps) => {
  const [search, setSearch] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<EffectCategory | "todos">("todos");
  ...
  return (
    <div className="flex flex-col h-full bg-background border border-purple-500/20 rounded-xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
      <div className="p-4 border-b border-border/50 flex items-center justify-between bg-muted/20">
        <div>
          <h3 className="text-sm font-bold uppercase tracking-widest text-purple-400">Selecionar Efeito</h3>
          <p className="text-[10px] text-muted-foreground">Escolha um efeito base para adicionar ao seu poder</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2">
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="text-right flex flex-col items-end gap-2">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="text-[10px] text-purple-400 font-bold uppercase tracking-tighter">Adicionar</span>
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-3">
              ...
              <div>
                <p className="text-sm font-medium text-muted-foreground">Nenhum efeito encontrado</p>
                <p className="text-xs text-muted-foreground/50">Tente ajustar sua busca ou categoria</p>
              </div>
            </div>
          )}
        </div>
