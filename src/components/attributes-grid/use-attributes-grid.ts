...
export function moveItemById<T extends { id: string }>(items: T[], fromId: string, toId: string): T[] {
...
export function updateItemById<T extends { id: string }>(items: T[], id: string, updates: Partial<T>): T[] {
...
export function useGenericGrid<T extends { id: string }>(
    ...
    const handleDragEnd = useCallback((event: DragEndEvent) => {
        ...
        if (active.id !== over?.id && over) {
            setItems((prev) => {
                const newItems = moveItemById(prev, active.id as string, over.id as string)
                onChange?.(newItems)
                return newItems
            })
        }
