...
function PowerBuilderModalContent({ onClose, onSave, editingPower }: PowerBuilderModalProps) {
  ...
  const [selectedEffects, setSelectedEffects] = useState<Effect[]>(editingPower?.effects || [])
  const [rank, setRank] = useState(editingPower?.rank || 1)
  const [selectedModifierInstances, setSelectedModifierInstances] = useState<ModifierInstance[]>(editingPower?.modifiers || [])
  const [selectedDescriptors, setSelectedDescriptors] = useState<string[]>(editingPower?.descriptors || [])
  const [customAction, setCustomAction] = useState<ActionType | null>(editingPower?.customAction || null)
  const [customRange, setCustomRange] = useState<RangeType | null>(editingPower?.customRange || null)
  const [customDuration, setCustomDuration] = useState<DurationType | null>(editingPower?.customDuration || null)
  ...
  const filteredExtras = useMemo(() => {
    ...
    const common = COMMON_EXTRAS.filter(m => {
      ...
      const appliesOk = !m.appliesTo || m.appliesTo.some(effectId => selectedEffects.some(e => e.id === effectId))
    ...
    const specific = (typeof EFFECT_SPECIFIC_EXTRAS !== 'undefined' ? EFFECT_SPECIFIC_EXTRAS : []).filter(m => (m.name.toLowerCase().includes(q) || m.description.toLowerCase().includes(q)) && (
      !!m.appliesTo && m.appliesTo.some(effectId => selectedEffects.some(e => e.id === effectId))
    ))
  ...
  const filteredFlaws = useMemo(() => {
    ...
    const specific = (typeof EFFECT_SPECIFIC_FLAWS !== 'undefined' ? EFFECT_SPECIFIC_FLAWS : []).filter(m => (m.name.toLowerCase().includes(q) || m.description.toLowerCase().includes(q)) && (
      !!m.appliesTo && m.appliesTo.some(effectId => selectedEffects.some(e => e.id === effectId))
    ))
  ...
  const toggleEffect = useCallback((effect: Effect) => {
    setSelectedEffects(prev => {
      ...
      const next = isSelected ? prev.filter(e => e.id !== effect.id) : [...prev, effect]
      ...
      setSelectedModifierInstances(current =>
        current.filter(inst => {
          ...
          return applies.some(effectId => next.some(e => e.id === effectId))
        })
      )
  ...
  const removeModifierInstance = useCallback((instanceId: string) => {
    setSelectedModifierInstances(prev => prev.filter(m => m.id !== instanceId))
  }, [])

  const updateModifierDescription = useCallback((instanceId: string, description: string) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-card border border-purple-500/20 rounded-xl w-full max-w-5xl h-[85vh] overflow-hidden flex flex-col shadow-2xl">
        {/* Header */}
        <PowerBuilderHeader isEditing={isEditing} step={step} onClose={onClose} />
