...
export function PowerCard({ power, onDelete, onEdit, isEditMode }: PowerCardProps) {
  ...
  const duration = power.customDuration || power.effects[0]?.duration || 'instantaneo'
    ...
    <div className="bg-background/30 border border-purple-500/20 rounded-lg overflow-hidden transition-all">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-sm font-semibold text-foreground">{power.name}</h4>
            <div className="text-xs text-muted-foreground flex flex-wrap items-center gap-1">
              {power.effects.map((e, i) => (
                <span key={i} className="flex items-center">
                  <Tip content={<div className="max-w-xs text-xs">{e.description}</div>}>
        ...
        <div className="px-3 pb-3 space-y-3 border-t border-purple-500/10">
          ...
          <div className="pt-3 space-y-1">
            {power.effects.map((effect, idx) => (
              <p key={idx} className="text-xs text-muted-foreground">
                <span className="font-medium text-foreground">{effect.name}:</span> {effect.description}
              </p>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="bg-background/40 p-2 rounded">
              ...
              <p className="text-xs font-medium text-foreground">{RANGE_LABELS[range]}</p>
            </div>
            <div className="bg-background/40 p-2 rounded">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Duração</span>
              <p className="text-xs font-medium text-foreground">{DURATION_LABELS[duration]}</p>
            </div>
          </div>
