...
interface DefensesContextType {
  ...
  updateDefense: (
    key: keyof NonNullable<CharacterDocument["defenses"]>,
    value: number,
  ) => void;
  markFieldDirty: (field?: string) => void;
}

const DefensesContext = createContext<DefensesContextType | undefined>(
  undefined,
);

export const DefensesProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const {
    state,
  ...
  const updateDefense = useCallback(
    (key: keyof NonNullable<CharacterDocument["defenses"]>, value: number) => {
      ...
      const baseDefenses = {
        ...
        resistencia: 0,
        vontade: 0,
      } as NonNullable<CharacterDocument["defenses"]>;
      updateDefenses((prev) => ({
        ...baseDefenses,
        ...(prev || {}),
        [key]: value,
  ...
  return (
    <DefensesContext.Provider
      value={{ defenses, updateDefense, markFieldDirty }}
    >
      {children}
    </DefensesContext.Provider>
  );
...
export function useDefenses() {
  ...
  if (!ctx)
    throw new Error("useDefenses must be used within a DefensesProvider");
  return ctx;
}
