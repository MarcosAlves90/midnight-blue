...
export function CharacterProvider({ children }: { children: React.ReactNode }) {
  const [selectedCharacter, internalSetSelectedCharacter] =
  ...
  useEffect(() => {
    ...
    return repo.onCharacterChange(selectedCharacter.id, (updatedChar) => {
      if (!updatedChar) return;

      // Só atualiza se houver mudança real de versão ou timestamp para evitar loops
      if (
        updatedChar.version !== selectedCharacter.version ||
        updatedChar.updatedAt.getTime() !==
          selectedCharacter.updatedAt.getTime()
      ) {
  ...
  const setSelectedCharacter = useCallback(
    (character: CharacterDocument | null) => {
      ...
      if (character) {
        CharacterStorageService.saveSelection(
          character,
