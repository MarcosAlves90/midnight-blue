...
export const PowersProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const updatePowers = useCallback(
    (action: React.SetStateAction<Power[]>) => {
      updateSheet(action);
    },
    [updateSheet],
  );
  ...
  return (
    <PowersContext.Provider value={value}>{children}</PowersContext.Provider>
  );
};
