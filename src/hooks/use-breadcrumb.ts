...
export interface BreadcrumbConfig {
    ...
    showHome?: boolean;
    dynamicRouteHandler?: (segment: string, index: number, segments: string[]) => string | null;
}
...
function isDynamicSegment(segment: string): boolean {
    ...
    return uuidRegex.test(segment) || numericRegex.test(segment) || hashRegex.test(segment);
}
...
function getRouteLabel(
    ...
    if (isDynamicSegment(segment)) {
        ...
        if (previousSegment) {
            const baseLabel = routeLabels[previousSegment] || 
